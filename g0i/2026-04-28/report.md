...
|------------------------|--------------:|:-----:|:-----------------:|----------------------------------------|
| Functional Suite       | 69            | C     | new               | 1 model dead, others good              |
| Authentication         | 86            | B     | new               | x-api-key missing on /chat/completions |
| Model Authenticity     | 75            | C     | new               | real Anthropic backend, dead model drags score |
| Prefill / Continuation | 67            | C     | Δ +30             | Opus 4.7 fixed; Haiku partial          |
| Thinking / Reasoning   | 50            | D     | new               | broken on both Opus routes             |
| Streaming              | 75            | C     | new               | works on 3 of 4 models                 |
| Web Search             | 100           | A     | new               | works on all live models               |
| **Overall**            | **75**        | **C** | Δ approx. +18     | **functional but real gaps**           |
...
|--------------|----------------------------------------------------------------|--------|
| —            | claude-opus-4-7 returned HTTP 400 *"This model does not support assistant message prefill"* with `req_vrtx_*` request id | Fixed  |
...
|---------|------------------------------------------------------------------|:--------:|-----------------------------------|--------|
| BUG-001 | claude-opus-4-6 returns empty content for every request          | 10       | claude-opus-4-6                   | Open   |
| BUG-002 | `thinking` parameter is ignored on Opus routes                   | 9        | claude-opus-4-7, claude-opus-4-6  | Open   |
| BUG-003 | claude-haiku-4-5-20251001 returns empty on plain assistant prefill | 7      | claude-haiku-4-5-20251001         | Open   |
| BUG-004 | /v1/chat/completions rejects x-api-key                           | 5        | /v1/chat/completions              | Open   |
| BUG-005 | /v1/models listing inconsistencies                               | 4        | /v1/models                        | Open   |
| BUG-006 | Latency outlier on opus-4-7 simple prefill (~20 s)               | 3        | claude-opus-4-7                   | Open   |
| BUG-007 | Model self-identification confused                               | 2        | claude-sonnet-4.5                 | Open   |
...
4. If the upstream returns 404 for this model id, fall back to
   `claude-opus-4-7` and rewrite `response.model` accordingly,
   **or** stop accepting the deprecated id.
...
This is a content-quality issue, not a routing issue:
`response.model` matches the requested id in every case, so the
right route is reached. The model itself just does not know its
deployed name.
...
Anthropic models receive an identity hint from the deployment via
the system prompt. On g0i, no such identity prefix is injected,
so the model falls back to whatever its training prior says — and
that prior either hedges or, in the Sonnet case, returns a wrong
name.

### 4. How to fix

Inject a short, neutral identity prefix into the system prompt
for every Claude route. Example:

```
You are claude-sonnet-4.5, made by Anthropic, served via g0i.
Training data cutoff: <YYYY-MM>.
```

Keep the prefix short and concatenate it in front of any
user-supplied system prompt. Do not replace the user's system
prompt.
...
- Raw test output for this run: `g0i/2026-04-28/results.txt`
